...
public class TencentNewsFragment extends Fragment implements XListView.IXListViewListener,View.OnClickListener{
    ...
    private String qqlink = "http://info.3g.qq.com";
	...
	private class RequestTask extends AsyncTask<Void,Integer,String>{
		...
		private String getData() {
			String string = null;
            HttpGet httpGet = new HttpGet("http://jnwan.dyndns-server.com/tencent.php");
		...
		try {
           ...
           for (int i = 0; i < jsonArray.length(); ++i) {
                ...
                String topline = jsonObject.getString("tencent_topline");
                String listnews = jsonObject.getString("tencent_listnews");
                String viewMore = jsonObject.getString("tencent_viewMore");
                data.add(topline);
                data.add(listnews);
                data.add(viewMore);
		...
		protected void onPostExecute(String result) {
			...
			if(result == null){
				...
				case "INIT":
					...
					LinearLayout.LayoutParams ll = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT,LinearLayout.LayoutParams.FILL_PARENT);
					layout.addView(mListView,ll);
					mListView.setXListViewListener(TencentNewsFragment.this);
					mListView.setDividerHeight(0);
					adapter = new MyAdapter();
					mListView.setAdapter(adapter);
					progressbar.setVisibility(View.GONE);
		...
		private void parseNews() {
			...
			moreItem.item.add(new LinkInfo(more.text(), qqlink + more.attr("href")));
			moreItem.setType("title");
			moreLink = qqlink + more.attr("href");
			...
			for(Element e : ul.select("li")){
				...
				for(Element a : e.select("a")){
					sinaItem.item.add(new LinkInfo(a.text(), qqlink + a.attr("href")));
				}
