...
public class SohuNewsFragment extends Fragment implements XListView.IXListViewListener,View.OnClickListener{
    ...
    private String sohulink = "http://m.sohu.com";
	...
	private class RequestTask extends AsyncTask<Void,Integer,String>{
		...
		private String getData() {
			String string = null;
            HttpGet httpGet = new HttpGet("http://jnwan.dyndns-server.com/sohu.php");
		...
		try {
           ...
           for (int i = 0; i < jsonArray.length(); ++i) {
                ...
                String topline = jsonObject.getString("sohu_topline");
                String listnews = jsonObject.getString("sohu_listnews");
                String viewMore = jsonObject.getString("sohu_viewMore");
		...
		protected void onPostExecute(String result) {
			...
			if(result == null){
				...
				case "INIT":
					...
					LinearLayout.LayoutParams ll = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT,LinearLayout.LayoutParams.FILL_PARENT);
					layout.addView(mListView,ll);
					mListView.setXListViewListener(SohuNewsFragment.this);
					mListView.setDividerHeight(0);
					adapter = new MyAdapter();
					mListView.setAdapter(adapter);
					progressbar.setVisibility(View.GONE);
		...
		private void parseNews() {
			Element doc = Jsoup.parse(data.get(2));
			Element more = doc.select("a").first();
			SinaItem moreItem = new SinaItem();
			moreItem.item.add(new LinkInfo(more.text(), sohulink+more.attr("href")));
			moreItem.setType("title");
			moreLink = sohulink+more.attr("href");
			localdata.add(moreItem);
			doc = Jsoup.parse(data.get(1));
			for(Element e : doc.select("div.it")){
				SinaItem sinaItem = new SinaItem();
				for(Element a : e.select("a")){
					sinaItem.item.add(new LinkInfo(a.text(), sohulink+a.attr("href")));
				}
		...
		private void parseTopline() {
			 ...
			 Element content = doc.select("section").first();
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(item == null){
				...
				eachView.setOnClickListener(new View.OnClickListener() {
					...
					@Override
					public void onClick(View v) {
						v.setBackgroundColor(Color.parseColor("#4caffd"));
						Intent intent = new Intent(context,MyWebActivity.class);
						intent.putExtra("link", moreLink);
