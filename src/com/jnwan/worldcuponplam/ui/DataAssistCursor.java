...
public class DataAssistCursor extends Fragment implements XListView.IXListViewListener,View.OnClickListener{
	...
	private void init() {
		if(data == null){
			...
			cursor = db.query("assist_table", new String[] { "_id", "rank", "player", "national_team", "club_team", "assist" }, null, null, null, null, null);
			adapter = new DataCursorAdapter2(context,R.layout.datashooter_item,cursor,new String[]{"rank", "player", "national_team", "club_team", "assist"},new int[]{R.id.datashooter_rank,R.id.datashooter_player,R.id.datashooter_nation,R.id.datashooter_club,R.id.datashooter_goal});			
     ...
     private String state;  
		...
		protected void onPostExecute(String result) {
			...
			if(result == null){
				...
				db.execSQL("delete from assist_table");
                for(int i = 0; i < data.size(); i++){
                	DataAssistDetail dd = data.get(i);
                	int rank = i + 1;
                	String name = dd.player;
                	String nation = dd.national_team;
                	String club = dd.club_team;
                	int assist = dd.assist;
                	ContentValues cv = new ContentValues();
                	cv.put("rank", rank);
                	cv.put("player", name);
                	cv.put("national_team", nation);
                	cv.put("club_team", club);
                	cv.put("assist", assist);
                	db.insert("assist_table", null, cv);                	
                }
				switch (state) {
				case "INIT":
					mListView = new XListView(context);
					...
					LinearLayout.LayoutParams ll = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT,LinearLayout.LayoutParams.FILL_PARENT);
					layout.addView(mListView,ll);
					mListView.setXListViewListener(DataAssistCursor.this);
					db = helper.getReadableDatabase();
					cursor = db.query("assist_table", new String[] { "_id", "rank", "player", "national_team", "club_team", "assist" }, null, null, null, null, null);
					adapter = new DataCursorAdapter2(context,R.layout.datashooter_item,cursor,new String[]{"rank", "player", "national_team", "club_team", "assist"},new int[]{R.id.datashooter_rank,R.id.datashooter_player,R.id.datashooter_nation,R.id.datashooter_club,R.id.datashooter_goal});			
					mListView.setAdapter(adapter);
